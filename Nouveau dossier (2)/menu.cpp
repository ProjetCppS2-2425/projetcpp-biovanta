    ...
    ui(new Ui::menu)
{
    ...
    stackedWidget->addWidget(client1interface);     // Index 4
     stackedWidget->addWidget(testBioInterface);
    ...
    QPushButton* btnRetourTestBio = testBioInterface->findChild<QPushButton*>("pushButton_9");
    ...
    if (btnRetourTestBio) {
        connect(btnRetourTestBio, &QPushButton::clicked, this, [this]() {
            qDebug() << "Retour test biologique cliqué";
            stackedWidget->setCurrentIndex(0);
        });
    } else {
        qDebug() << "Erreur: Bouton retour test biologique non trouvé!";
    }
}
...
void menu::on_equip_5_clicked()
{
    qDebug() << "Affichage interface test biologique";
    stackedWidget->setCurrentIndex(5);  // Page test biologique
}
